...
bool FFNN_node::set_input(double value,
                     uint input_number)
{
    if(input_number < 0 || input_number >_inputcount)
        return false;
    else
    {
...
{
    for(uint index = 0; index < _inputcount; index++)
    {
...
void FFNN_network::train(double abort_time)
{
    ...
    //FILE* outpfile = fopen("/home/wolfi/bla/output","w");
    ...
    while(1)
    {
        for(uint i = 0; i < _training_data.size(); i++)
        {
            ...
            //save("/home/wolfi/bla/temp1.save");
            ...
            learn(0.6);
            //cout << "MSE = " << calc_MSE() << endl;
            //save("/home/wolfi/bla/temp2.save");
        }
...
void FFNN_network::normalize_training_set()
{
    ...
    for(uint i = 0; i < _N_input_layer; i++)
    {
        ...
        if(min_input[i] == max_input[i])
            normi = new Normalizer(min_input[i]-0.5,max_input[i]+0.5);
        else
            normi = new Normalizer(min_input[i],max_input[i]);
    ...
    for(uint i = 0; i < _N_output_layer; i++)
    {
        if(min_output[i] == max_output[i])
            normi = new Normalizer(min_output[i]-0.5,max_output[i]+0.5);
        else
            normi = new Normalizer(min_output[i],max_output[i]);
...
// including network size, Max_MSE, weights, biases and the normalizer data
bool FFNN_network::load(const char *file)
{
