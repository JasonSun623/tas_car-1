...
ODOMVELCALCULATOR::ODOMVELCALCULATOR()
{
    ...
    // used matlab script to determine filter parameters A and B
...
void ODOMVELCALCULATOR::odomCallback(const nav_msgs::Odometry::ConstPtr& odom_without_vel)
{
    ...
    // current and last rotation angle around z
    rotZ_old = rotZ;
    rotZ = RPY(2);
    ...
    if(m_old_odometry_valid)
    {
        ...
        // calculate velocities using current and last position
        ...
        // make sure the change of the angle is continous
        if((vel(5) - M_PI) > 0) 
        {
            vel(5) = vel(5) - 2 * M_PI;
        }    
        ...
        // don't filter the rotation velocity
